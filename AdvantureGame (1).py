...
|__|__|LT|EN|DT|__|EN|LT|__|__|__|__|__|__|__|__|__|__|
...
class player:
    class Player:
        def __init__(self):
            self.inventory = [gameItems.Spear(),gameItems.pistol223() ]
        ...
        def attack(self,w):
            ...
            if best_weapon.name == "Explosive":
              self.inventory.remove(best_weapon)
            if best_weapon.name == ".223 pistol":
               best_weapon.rounds-=1
               printNow("Rounds Left: "+str(best_weapon.rounds))
...
def choose_action(w,room, player):
    ...
    while not action:
        ...
        if action_input is None:
            return
        elif action_input.find("/quit")==0 or action_input.find("/q")==0 or action_input.find("/Q")==0:
